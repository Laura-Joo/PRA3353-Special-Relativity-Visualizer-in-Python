...
def draw_t_ticks(v: float, prefix: str):
    ...
    for i in range(1, amount_of_t_ticks + 1):
...
def draw_light_cones(event_number):
    ...
    positive_x_endpoint = x_coordinate + LIGHTCONE_SCALE
    positive_y_endpoint = y_coordinate - LIGHTCONE_SCALE

    negative_x_endpoint = x_coordinate - LIGHTCONE_SCALE
    negative_y_endpoint = y_coordinate - LIGHTCONE_SCALE
    ...
    positive_lightcone.setAttribute("x1", str(x_coordinate))
    positive_lightcone.setAttribute("y1", str(y_coordinate))
    ...
    negative_lightcone.setAttribute("x1", str(x_coordinate))
    negative_lightcone.setAttribute("y1", str(y_coordinate))
...
def remove_lightcones(event_number):
    ...
    if positive_lightcone is not None:
        positive_lightcone.remove()

    if negative_lightcone is not None:
...
def handle_lightcone_checkbox(event):
    ...
    x = float(point.getAttribute("cx"))
    y = float(point.getAttribute("cy"))
...
def update_event_position(event_number):
    ...
    checkbox = document.getElementById(f"lightcone_checkbox_{event_number}")
